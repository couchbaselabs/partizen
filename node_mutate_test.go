...
func TestEmptyMutate(t *testing.T) {
	ksl, err := rootKeySeqLocProcessMutations(nil, nil, nil, 15, 32, nil)
	if err != nil {
	...
	ksl, err = rootKeySeqLocProcessMutations(rootKeySeqLoc, nil, nil, 15, 32, nil)
	if err != nil {
		t.Errorf("expected ok, err: %#v", err)
	}
	if ksl != nil {
	...
	m := []Mutation{}
	ksl, err = rootKeySeqLocProcessMutations(nil, m, nil, 15, 32, nil)
	if err != nil {
		t.Errorf("expected ok, err: %#v", err)
	}
	if ksl != nil {
...
func TestMutationsOn1Val(t *testing.T) {
	...
	for _, keyNotThere := range []string{"x", "0", "aa", ""} {
		...
		ksl2, err := rootKeySeqLocProcessMutations(ksl2, m, nil, 15, 32, nil)
		if err != nil {
			t.Errorf("expected ok, err: %#v", err)
		}
		if ksl2 == nil {
	...
	ksl3, err := rootKeySeqLocProcessMutations(ksl2, m, nil, 15, 32, nil)
	if err != nil {
		t.Errorf("expected ok, err: %#v", err)
	}
	if ksl3 != nil {
...
func TestMutationsOn2Vals(t *testing.T) {
	...
	ksl4, err := rootKeySeqLocProcessMutations(ksl3, m, nil, 15, 32, nil)
	if err != nil {
		t.Errorf("expected ok, err: %#v", err)
	}
	if ksl4 != nil {
...
func TestMutationsDepth(t *testing.T) {
	...
	for c, i := 0, iStart; c < n; c, i = c+1, delta(c, i) {
		...
		m[0].Val = Val(m[0].Key)
		rootKeySeqLoc, err =
			rootKeySeqLocProcessMutations(rootKeySeqLoc, m, nil, 2, 4, nil)
		if err != nil {
